...
import Loginn from '../../pages/loginn.js'; // make sure path is correct
...
const Navbar = () => {
  ...
  const [mobileOpen, setMobileOpen] = useState(false);
  const [anchorEl, setAnchorEl] = useState(null);
  const [showUserlogin, setShowUserlogin] = useState(false); // modal visibility
  ...
  const navItems = [
    ...
    { text: 'User', path: '/user' },
    { text: 'Admin', path: '/admin/dashboard' },
    { text: 'Login', action: () => setShowUserlogin(true) },
    { text: 'Track Issue', path: '/track-issue' },
  ];
  ...
  return (
    <>
      ...
      {/* Login Modal */}
      <Loginn show={showUserlogin} onClose={() => setShowUserlogin(false)} />
    </>
  );
};
